...
public class SwipeListViewTouchListener implements View.OnTouchListener {
    ...
    public void setRightDistance(float righDistance) {
        this.righDistance = righDistance;
    }
...
//        }
    ...
    private void generateRevealAnimate(final View view, final boolean swap, final boolean swapRight, final int position) {
        ...
        Log.d("++", "opened.get(position) = " + opened.get(position));
        Log.d("++", "openedRight.get(position) = " + openedRight.get(position));
        Log.d("++", "swap = " + swap);
        Log.d("++", "swapRight = " + swapRight);

        if (opened.get(position)) {
            if (!swap) {
                moveTo = openedRight.get(position) ? (int) (viewWidth - rightOffset) : (int) (-viewWidth + leftOffset);

                if (righDistance > 0 || leftDistance > 0)
                    moveTo = openedRight.get(position) ? (swapRight ? (int) righDistance : (int) (-viewWidth + righDistance)) :
                            (swapRight ? (int) righDistance : -(int) leftDistance);
            } else {
                if (openedRight.get(position)) {
                    moveTo = swapRight ? (int) rightOffset : 0;
                } else {
                    moveTo = swapRight ? (int) rightOffset : 0;
                }
            }
            ...
            if (swap) {
                moveTo = swapRight ? (int) (viewWidth - rightOffset) : (int) (-viewWidth + leftDistance);

                // XXX max distance
                moveTo = righDistance > 0 ? (swapRight ? (int) righDistance : moveTo) : moveTo;
                moveTo = leftDistance > 0 ? (!swapRight ? (int) -leftDistance : moveTo) : moveTo;
            }
        }
...
//                }
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if (swiping && downPosition != ListView.INVALID_POSITION) {
                    if (opened.get(downPosition)) {
                        ...
                        if (righDistance > 0 || leftDistance > 0)
                            deltaX = openedRight.get(downPosition) ? righDistance : -leftDistance;
                        else
    ...
    public void move(float deltaX) {
        ...
        if (opened.get(downPosition)) {
            ...
            if (leftDistance > 0 || righDistance > 0)
                posX = 100;//openedRight.get(downPosition) ? -righDistance : leftDistance;
            else
