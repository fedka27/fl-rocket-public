...
public class CountryMaster
{
	...
	private String getCountryName(String isoCode)
	{
		...
		String l = locale.getDisplayCountry().trim();
		if (TextUtils.isEmpty(l))
		{
			locale = new Locale("", isoCode);
			l = locale.getDisplayCountry().trim();
		}

		//locale.getDisplayCountry().trim();

		return l;
	}

	/**
	 * Uses libphonenumber library to fetch a {@link Phonenumber.PhoneNumber} object, from
	 ...
	 * @return number		{@link Phonenumber.PhoneNumber}
	 * 
	 * @see <a href="https://github.com/googlei18n/libphonenumber">https://github.com/googlei18n/libphonenumber</a>
	 */
	public Phonenumber.PhoneNumber getPhoneNumber(String phoneNumber)
	{
	...
	public void debug()
	{
		...
		{
			String out = String.format(Locale.getDefault(), "[%d] [%s] prefix: %s, name: %s, resId: %d", i++, country.mCountryIso,
					country.mDialPrefix, country.mCountryName, getCountryFlagImageResource(country.mCountryIso));
			Log.d(TAG, out);
		}
	}
}
